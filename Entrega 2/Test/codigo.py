		...
		if "CODE" in line[0]:
			...
			if len(line)==1:
				if ":" in line[0]:
					...
					print("\""+ str(assembly_dict[line[0]]) + "\"")
			else:
				line1=line[1].split(",")
				if len(line1)==1:
					if line1[0] in "AB":
						## Estructura: bin_text = lo que habia antes en el bin_text
						## '0000000000000000' es si hay un A o B como segundo argumento de la instruccion. En otro caso, se da elb literal o la direccion de memoria en binario
						...
						## \n para dar un enter
						bin_text = bin_text +'0000000000000000' + assembly_dict[line[0]+" "+line1[0]] + '\n'
						print(str(assembly_dict[line[0]+" "+line1[0]]) + "\"")
					elif "(" in line1[0]:
						line1[0] = line1[0][1 : len(line1[0]) - 1]
						bin_text = bin_text + direcciones_en_bin[direcciones[line1[0]]]  + assembly_dict[line[0]+" (Dir)"] + '\n'
						print("\"" + str(assembly_dict[line[0]+" (Dir)"]) + "\"")
					else:
						...
						## el resto todo igual
						try: 
							bin_text = bin_text + convertir_binario(int(direcciones[line1[0]])) + assembly_dict["JMP (Dir)"] + '\n'
						except:
					...
					if line1[0] in "AB":
						if "(" in line1[1]:
							...
							bin_text = bin_text + direcciones_en_bin[direcciones[line1[1]]] + assembly_dict[line[0]+" "+line1[0]+",(Dir)"] + '\n'
							print(assembly_dict[line[0]+" "+line1[0]+",(Dir)"])
						elif line1[1] in "AB":
							bin_text = bin_text + '0000000000000000' +  assembly_dict[line[0]+" "+line1[0]+","+line1[1]] + '\n'
							print(assembly_dict[line[0]+" "+line1[0]+","+line1[1]])
						else:
							lit = convertir_binario(int(line1[1]))
							bin_text = bin_text + lit + assembly_dict[line[0]+" "+line1[0]+",Lit"] + '\n'
							print(assembly_dict[line[0]+" "+line1[0]+",Lit"])

					elif line1[1] in "AB":
						if "(" in line1[0]:
							bin_text = bin_text + '0000000000000000' +  assembly_dict[line[0]+" (Dir),"+line1[1]] + '\n'
